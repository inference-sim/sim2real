...
**Stage 3 implication:** Do NOT implement `Start()` or `Stop()` methods unless the `plugin.Plugin` interface definition at the pinned commit requires them (which it does not at commit `4cd7046`).
