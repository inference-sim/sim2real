...
>
> **IMPORTANT — UNVERIFIED FIELD HALT CONDITION (Stage 3 scope — Stage 3 MUST execute this during code generation):** Metric field names marked `// UNVERIFIED` in this template have NOT been confirmed against the actual `fwkdl.Metrics` struct at the pinned dependency version (`gateway-api-inference-extension v0.0.0-20260128235548-fd30cb97714a`). Before generating code, Stage 3 MUST:
...
func (s *EvolvedScorer) Score(ctx context.Context, _ *scheduling.CycleState, request *scheduling.LLMRequest, endpoints []scheduling.Endpoint) map[scheduling.Endpoint]float64 {
	...
	for _, endpoint := range endpoints {
		...
		//
		// ⚠ DOUBLE-COUNTING GUARD (F-10): If RunningRequestCount is UNAVAILABLE and
		// InFlightRequests falls back to RunningQueueSize, this formula becomes
		// WaitingQueueSize + 2*RunningQueueSize (double-counting RunningQueueSize).
		// Stage 3 MUST detect this case (check HALT CONDITION field resolution results)
		// and either use a different proxy or adjust the composite computation.
		// See README.md § Cross-PR Contracts item 4 for the full F-10 specification.
