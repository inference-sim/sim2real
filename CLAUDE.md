...
**`/sim2real-translate`** — DISABLED as of step-1; re-enabled in step-2 (issue [#468](https://github.com/inference-sim/sim2real/issues/468)). The skill reads `translations/<hash>/skill_input.json` written by `sim2real translate` and writes `generated/<algo>/{cmd,pkg,<algo>_output.json,<algo>_config.yaml}` per algorithm.
