...
Infrastructure choices that BLIS doesn't model: gateway type and sizing, connection pool settings, baseline (load-aware) scorer config, model deployment constants (auth secret, service port, `prefill.create`), and `observe.noise_runs`. Edit this file when switching gateway providers or tuning cluster-level parameters. The file is committed and shared across algorithm runs.
