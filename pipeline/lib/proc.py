...
def run(
    ...
    *,
    ...
    directory. ``input`` feeds stdin. ``timeout`` (seconds) caps the call and
    raises :class:`subprocess.TimeoutExpired` when exceeded; ``None`` means no
    limit — callers such as ``deploy.run``/``cluster_ops._run`` pass their own
    default (120 s, #694). This is the single process-exec seam for the
    pipeline; tests may monkeypatch this function to intercept invocations.
    """
