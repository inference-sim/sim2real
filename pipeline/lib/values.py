...
# ── Post-merge transformations (used by deploy.py) ───────────────────────────
...
def _apply_request_multiplier(merged: dict) -> dict:
    ...
    if not isinstance(multiplier, (int, float)):
        print(f"ERROR: observe.request_multiplier must be a number, "
    ...
    for wl in workloads:
        ...
        try:
            ...
            print(f"WARNING: could not parse spec YAML for workload "
...
def merge_values(
    ...
    Used by deploy.py for EPP image re-injection. Prepare.py no longer uses this.
    """
