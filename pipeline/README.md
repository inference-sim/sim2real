...
**Completeness marker.** An iteration is skipped as up-to-date only when it carries `i<N>/.collect_complete`, that marker is not stale relative to the remote `trace_data.csv` mtime, and its scope covers what this collect is asking for. Scope matters because `--skip-logs` establishes only "complete apart from `server_logs`": the marker records what it left out, and covers a request only when everything it omitted is also omitted now. So `collect --skip-logs` followed by a plain `collect` — the sequence the >1 GB size prompt actively recommends — re-examines the iteration and fetches `server_logs`, instead of reporting it up to date and never transferring the logs at all. A marker with no scope field predates this check and is not honoured; the iteration is re-examined once at the cost of one inventory and zero transfers, then the marker is rewritten with the field. When a marker *is* stale — the remote advanced past what the marker was written against — the whole inventory is refetched rather than just the size delta, because an in-place rewrite can leave sizes unchanged. That refetch needs positive evidence the remote moved: a marker whose mtime cannot be compared (missing probe, unparseable value) does not trigger it. The marker is written **only** after every file in the remote inventory is present locally at the matching size, so a partially-copied iteration has no marker and is resumed rather than skipped. Before #885 completeness was inferred from `trace_data.csv`'s mtime alone; that file is copied early in the tar stream, so it survived a truncation and the next collect reported the partial iteration as up to date — silently retaining truncated logs across repeated "successful" collects. Iterations collected before #885 have no marker and are re-examined once; the delta against a complete local tree is empty, so that costs one remote inventory and zero transfers. Deleting the marker forces a re-collect of that iteration.
