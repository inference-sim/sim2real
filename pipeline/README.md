...
- `.collect_complete` — per-iteration completeness marker written locally by `collect`, not present on the PVC (JSON: `completed_at`, `file_count`, `byte_count`, `inventory_sha256`, `remote_mtime`). Its presence is the only evidence `collect` accepts that an iteration transferred in full (issue #885). Delete it to force a re-collect of that iteration.
