...
def _preflight_check_values(values_path: "Path", namespace: str, phase: str,
                            helm_path: str | None = None) -> list:
    ...
    if phase == "treatment" and helm_path:
        # Walk the dotted helm_path to verify treatment config exists in values.yaml
        ...
        if not node:
            errors.append(
                f"FAIL: treatment config at '{helm_path}' is empty — "
                "treatment config must be generated by Stage 3."
            )
...
def cmd_preflight(args: "argparse.Namespace") -> int:
    ...
    helm_path = getattr(args, "helm_path", None)
    build_command = getattr(args, "build_command", None)

    # --- values-only checks (no kubectl) ---
    val_errors = _preflight_check_values(values_path, namespace, phase, helm_path=helm_path)
    for e in val_errors:
    ...
    if phase == "treatment":
        # Determine build command: use --build-command if provided, else default
        if build_command:
            import shlex
            build_cmd = shlex.split(build_command)
        else:
            build_cmd = ["go", "build", "./..."]
        ...
        checks.append(("Treatment plugin builds", ok, detail))
...
def cmd_merge_values(args: "argparse.Namespace") -> int:
    ...
    # Build the treatment config key path from --helm-path (default: scorer path)
    helm_path = getattr(args, "helm_path", None) or \
        "stack.gaie.treatment.helmValues.inferenceExtension.pluginsCustomConfig"
    helm_keys = helm_path.split(".")
...
def main():
    ...
    p_pf.add_argument("--helm-path", dest="helm_path", default=None,
                      help="Dotted path to treatment config key in values.yaml "
                           "(from transfer manifest config.helm_path)")
    p_pf.add_argument("--build-command", dest="build_command", default=None,
                      help="Build command to verify plugin compiles "
                           "(e.g. 'go build ./...'). Defaults to 'go build ./...'")
    ...
    p_mv.add_argument("--helm-path", dest="helm_path", default=None,
                      help="Dotted path to treatment config key to validate "
                           "(default: stack.gaie.treatment.helmValues.inferenceExtension.pluginsCustomConfig)")
